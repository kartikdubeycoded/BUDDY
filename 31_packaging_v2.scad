...
module packaging_v2(){
 ...
 for(a=[0:90:270]) rotate([0,0,a]) pod_v2(CAM_INNER_R,CAM_D,CAM_W,CAM_H,0);
 for(a=[90,270]) rotate([0,0,a]) pod_v2(BAT_INNER_R,BAT_D,BAT_W,BAT_H,0);
 for(a=[45,135,225,315]) rotate([0,0,a]) pod_v2(PCB_INNER_R,PCB_D,PCB_W,PCB_H,0);
}
module pod_v2(r,d,w,h,z){ translate([r+d/2,0,z]) cube([d,w,h],center=true); }
